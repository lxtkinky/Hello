...
+ (NSArray<NSTextCheckingResult *> *)matchReg:(NSString *)text reg:(NSString *)reg {
    if (text == nil || text.length == 0) {
        return nil;
    }
    
    NSString *pattern = reg;// 匹配中文或英文
...
+ (NSString *)hmacSha256:(NSString *)hashString hmacKey:(NSString *)key{
    ...
    NSData *paramData = [hashString dataUsingEncoding:NSUTF8StringEncoding];
...
//    NSString *salt = @"DJhCZACPX{9,KWcOO0YJAA[DB&qVVsPT";
    NSString *salt = @"3FwU5)_8:!X[?S93a9Gua@Wyu-$WYud.";
    ...
    if (token) {
        ...
        if (/*!IS_LOGIN*/ token == nil) {
            signatureStr = [NSString stringWithFormat:@"signature=\"%@\"",base64Hash];
        }else{
            signatureStr = [NSString stringWithFormat:@"signature=\"%@\" Bearer=\"%@\"",base64Hash,token];
        }
    }else{
        signatureStr = [NSString stringWithFormat:@"signature=\"%@\"",base64Hash];
    }
